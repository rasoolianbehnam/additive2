...
from additive2.plotEllipsoid import EllipsoidTool, Ellipsoid
...
def min_max_scale(x, new_min=0, new_max=255, dtype='float32', return_stats=False):
...
def find_pores_2(img, mask, kernel_size, pore_area_ratio):
    ...
    pore_img_ = ((smoothed_img[k2:-k2, k2:-k2] < (means - 2.1 * std)) * mask[k2:-k2, k2:-k2]).astype('uint8')
    dilated_pore_img = cv2.dilate(pore_img_, np.ones((1, 1)))

    pore_img = get_good_contours(dilated_pore_img, img_area, pore_area_ratio, pore_img_)
    return pore_img


def get_good_contours(dilated_pore_img, img_area, pore_area_ratio, pore_img_):
    _, contours, hierarchy = cv2.findContours(dilated_pore_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    good_contours = [cnt for cnt in contours if len(cnt) / img_area > pore_area_ratio]
...
def get_clusters(pore_img):
    ...
    # if two pixels are adjacent, only the first one will be set to 1 and the rest will be zero
    # this makes use of the fact that np.where lists pixels that are adjacent to each other.
...
def get_pores(pores_img: np.ndarray, are_pts):
    """
    :param pores_img: a 2d binary image representing pores or the coordinates corresponding to pores
    :param are_pts: A flag indicating whether pores_img is an image or coordinates
    :return: a tuple (x, y) where y is the index of non-zero pixels in pores_img and x is
...
def find_matching_pores(p1: np.ndarray, p2: np.ndarray):
    """
    :param p1: a 1d array representing the coordinates corresponding to pores in image 1
    :param p2: a 1d array representing the coordinates corresponding to pores in image 2
    :return: two 1d arrays: p1_p2_map and p2_p1_map. p1_p2_map[i] refers to a coordinate index in image 2 where
...
class PoreFeatures:
    ...
    def __init__(self, pts_in: np.ndarray, expand: bool = True):
        ...
        expand : Whether to add 1 to each combination of dimensions to make pore 3d
        """
        ...
        if expand:
            ...
            for i in range(1, n_dims + 1):
                ...
                for comb in combs:
                    ...
                    x[:, comb] += 1
                    pts.append(x)
            self.pts = pd.DataFrame(np.concatenate(pts, axis=0)).drop_duplicates().values
        else:
            self.pts = pts_in
...
def get_feature_pairs(cluster_map: List[Tuple[int, int]], pts1: np.ndarray, cls1: np.ndarray,
                      pts2: np.ndarray, cls2: np.ndarray) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Parameters
    --------------
    cluster_map
    pts1 : each row must be a (2d or 3d) point
    cls1:  1d array the same # of rows as pts1 which assings a cluster to each point
    pts2 : each row must be a (2d or 3d) point
    cls2:  1d array the same # of rows as pts1 which assings a cluster to each point

    Returns
    ------------
    Each dataframe the same size as input points and clusters where each row lists the pore features
    """
    mid_features_ = []
    high_features_ = []
    ...
    features1 = pd.DataFrame(mid_features_).reset_index().rename(columns={'index': 'cluster'})
    features2 = pd.DataFrame(high_features_).reset_index().rename(columns={'index': 'cluster'})
